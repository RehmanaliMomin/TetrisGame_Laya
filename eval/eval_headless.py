...
def run(policy, games: int, mask: bool, max_pieces: int, objective: str = "eltetris"):
    ...
    return {
        ...
        "avg lines": statistics.mean(lines), "best": max(lines), "tetrises": statistics.mean(tetrises),
        "avg pieces": statistics.mean(pieces),
...
def main():
    ...
    ap.add_argument("--teacher", default="eltetris", choices=["eltetris", "tetris"],
                    help="objective the agreement columns are measured against")
    a = ap.parse_args()
    rows = []
    for name in a.policy.split(","):
        print("running %s ..." % name, flush=True)
        rows.append(run(make_policy(name), a.games, not a.no_mask, a.max_pieces, a.teacher))
    cols = list(rows[0])
    print("\n| " + " | ".join(cols) + " |\n|" + "---|" * len(cols))
    for r in rows:
